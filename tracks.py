    ...
    if (args.kernel):
        ...
        if args.mitigation is None:
            ...
            if args.mitigation == 'gcov':
                ...
                target = f'tracks/linux-{args.mitigation}'
            else:
                assert args.level is not None, '--level required'
                cmd += f"--build-arg COMPILER='LLVM=1' "
                target = f"linux-{args.mitigation}-{args.level}"
        cmd += f"-t {target}-build -f Dockerfile.kernel ."
        print('exec:', cmd)
        status = os.system(cmd)
        cmd = f"docker build -t {target} --build-arg BUILD_IMAGE={target}-build "
        cmd += "--no-cache -f Dockerfile.kernel ."
    else:
